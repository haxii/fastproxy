...
func (r *Routers) Set(method, host, path string, handle HandleFunc) {
	if r.routers == nil {
		r.routers = make(map[string]*router)
	}
	if r.globList == nil {
		r.globList = make([]string, 0)
	}
	_router := r.routers[host]
	if _router == nil {
		_router = new(router)
		r.routers[host] = _router
	}
	_router.Set(method, path, handle)
	r.globList = append(r.globList, host)
}

func (r *Routers) GetHandleFunc(method, host, path string) (HandleFunc, Params) {
	for _, hostGlob := range r.globList {
		if glob(hostGlob, host) {
...
// Routers radix tree based on github.com/julienschmidt/httprouter
type router struct {
	trees map[string]*node
}

// Set registers a new request handle with the given path and method.
//
...
func (r *router) Set(method, path string, handle HandleFunc) {
	if path[0] != '/' {
...
// Get gets a request handle with the given path and method.
func (r *router) Get(method, path string) (HandleFunc, Params) {
	root := r.trees[method]
	if root == nil {
		// no specified method are find, try the wildcard one
		root = r.trees["*"]
	}
	if root != nil {
...
func (n *node) insertChild(numParams uint8, path, fullPath string, handle HandleFunc) {
